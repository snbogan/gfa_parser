...
def parse_cigar_overlap(cigar):
    ...
    Example: '13M' -> 13, '22M1D9M' -> 31, '6M2I8' -> 14
    """
    ...
    with open(gfa_file, 'r') as f:
        for line in f:
            if line.startswith('S'):
                ...
                overlap = parts[5]  # CIGAR string like '13M', '22M1D9M', '6M2I8'
                ov_len = parse_cigar_overlap(overlap)
                if reverse_edges:
                    graph.add_edge(to_node, from_node)
                    edges[(to_node, from_node)] = (to_strand, from_strand, ov_len)
                else:
...
def get_paths_undirected(graph, start, end):
    ...
    if end not in graph:
        sys.exit(f"Error: End node '{end}' not found in graph nodes.")

    undirected = graph.to_undirected()
    try:
        paths = list(nx.all_simple_paths(undirected, source=start, target=end))
    except nx.NetworkXNoPath:
        sys.exit(f"No path found between start '{start}' and end '{end}' ignoring edge direction.")

    if not paths:
        sys.exit("No paths found ignoring direction.")
...
def main():
    ...
    parser.add_argument('--reverse_edges', action='store_true',
                        help='Reverse edge directions when building graph')
    ...
    graph, sequences, lengths, depths, edges = parse_gfa(args.gfa, reverse_edges=args.reverse_edges)

    print(f"Graph nodes sample: {list(graph.nodes)[:20]}")
    print(f"Graph edges sample: {list(graph.edges)[:20]}")
    print(f"Start node: {start}")
    print(f"End node: {end}")

    filtered_unitigs, paths = get_paths_undirected(graph, start, end)
